...
- (id)initWithFrame:(NSRect)frame isPreview:(BOOL)isPreview
{
    ...
    if (self) {
		...
		scores[0] = [date minuteOfHour];
		scores[1] = [date hourOfDay];
		[self resetBall];
...
- (void)dealloc
{
	[configurePanel release];
	[configureObjController release];
	[launchSound release];
	[reboundSound release];
...
- (void)startAnimation
{
	...
	if( shouldAnimate )
		idleTimer = [[NSTimer scheduledTimerWithTimeInterval: kIdleResetInterval target: self selector: @selector( doIdle: ) userInfo: nil repeats: YES] retain];
}
	...
	if( bSpeed.x > 0 )
	{
		if( bPos.x + ballSize/2.0 > rFront )
		{
			...
			if( bPos.y >= miny && bPos.y <= maxy )
			{
				...
				bSpeed.y = copysignf( MIN( fabsf( bSpeed.y ), maxBallSpeed ), bSpeed.y );
				if( hasSound )
					[reboundSound play];
			}
			else
